...
function insert_update_classified($control){
  ...
  array_walk($_FILES, function($value, $idx) use($member_id, &$file_array_idx, &$four_files) {
   ...
   $four_files[$file_array_idx][2] =  '../images/classified/img_' . $member_id . '_' . $value['name'];
   $four_files[$file_array_idx][3] =  'images/classified/img_thn_' . $member_id . '_'. $value['name'];
  ...
  if($formValid){
    //member_id, control_number, adline, description, phone, email, island_id, pricing, image_1, image_2, image_3, image_4, active
    $insert_array = [];
    ...
    if($formStatus === 'add'){
        $insert_result = insert_classified($insert_array);
        $return_array[0] = $insert_result ;
    }

    if($formStatus === 'edit'){
        $insert_result = update_classified($insert_array);
        $return_array[0] = $insert_result ;
    }
